...
class Employee extends Eloquent {
    ...
    public function get_absence_data($month, $year) {
		...
		try{
			...
			$department = $this->department()->first(); //->allowance();
			...
			for ($i = $start_time; $i <= $end_time; $i = strtotime("+1 day", $i)) {
				...
				if ($current_day != 0 && MyDate::is_holiday(date('Y-m-d', $i))!=1) {//hari minggu libur jd tdk dihitung
					...
					*/
					...
					if(!is_null($in)){//cek sekali lagi apalah in sdh terisi
